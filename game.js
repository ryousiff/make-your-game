...
const gameOverButton = document.getElementById("restart-button");
...
function updateTime() {
  ...
  const seconds = Math.floor((diff % 60000) / 1000) // Convert to seconds
    .toFixed(0)
    .padStart(2, "0");
...
function showFinalTime() {
  gameOverTimeDisplay.innerHTML = `Time Taken: ${timeDisplay.innerHTML}`;
}
...
function restartGame() {
  ...
  // Clear the grid except the bottom row with 'taken' class
  squares.forEach((square, index) => {
    if (!square.classList.contains("taken") || index < squares.length - width) {
      square.classList.remove("tetromino");
...
continueButton.addEventListener("click", pause);
